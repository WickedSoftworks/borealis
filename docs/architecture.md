...
resolution to `resolveExpiry()`, password hashing to `hashSharePassword()`. The
things worth unit-testing therefore live in `lib/` and are testable without a
request: `lib/permissions.test.ts` and `lib/crypto/*.test.ts` do exactly that.
...
unlock cookie at once. Revocation needs no server-side session store. (The
feature is currently unreachable — no endpoint can change a share's password;
roadmap item 7.)
...
Accounting happens inside `after()` so it never delays the response: one
transaction increments `downloadCount` and `egressUsedBytes` and writes the
`ShareAccess` row. If `notifyOnDownload` is set, a `NOTIFY_DOWNLOAD` job is
enqueued — and then dropped, because the worker has no handler for it (roadmap
item 2).

Both routes read the whole object into a `Buffer` and hand it to `new Response`.
For a product that advertises multi-gigabyte uploads this is the sharpest edge
in the codebase: a 4 GB download allocates 4 GB, there is no `Range` support so
nothing can be resumed or seeked, and media preview is blocked on the same
absence. `StorageProvider` has no `stream()` yet. Roadmap item 9 is the fix, and
it changes egress accounting too — bytes served must then be counted from the
stream rather than from `buffer.byteLength`.
...
- Downloads buffer whole files and have no `Range` support (item 9). This is the
  one that contradicts a headline promise.
- `NOTIFY_DOWNLOAD` is enqueued and silently discarded (item 2).
- Shares are immutable after creation — there is no `PATCH` (item 7).
